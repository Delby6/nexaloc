// src/pages/notifications/OperatorNotifications.jsx
...
export default function OperatorNotifications() {
  ...
  function getCategory(n) {
    return (n.category || "network").toLowerCase();
  }
  ...
  if (loading || !user) {
    return (
      <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center">
        Loading operator notifications...
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2 text-slate-800 dark:text-white">
            ⚙️ Operator Console Inbox
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            System alerts, network updates, and operational AI insights.
          </p>
        </div>
              ...
              className={`p-4 rounded-xl border flex justify-between gap-4 transition-all
                ${
                  n.read
                    ...
                    : "bg-white dark:bg-slate-900 border-lime-500 shadow-sm shadow-lime-900/30"
                }`}
            >
              <div
